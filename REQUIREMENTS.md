...
* **Configurable Templates & Interactive Editor:** Standardized verbiage is stored in `config.json`. The application provides an advanced **Email Template Editor** (Step 5 in the TUI) allowing coaches to manage templates, signatures, and dashboard mapping without manual JSON editing.
* **Initial Outreach / Discovery:** Draft an email to ask the student about their goals, what they hope to work on, and their current experience level. Mapping to the dashboard's "Outreach" column is handled by setting the template type to `initial`.
